...
def detect_fire(
    ...
    for pre_idx, pre_item in enumerate(pre_fire_boxes):
        ...
        if pre_item.matched:
            ...
            x2, y2 = x1 + w, y1 + h
            ext_xxyy = sx1, sx2, sy1, sy2
            shape_id_best, coord_best, weight_best, im_best = fire_locate(img_bgr, (x1,y1,x2,y2), global_offset_x=0, global_offset_y=0, ext_xxyy=ext_xxyy, ret_all=False, path_idx=path_idx)
            if im_best is not None:
                im4_ret = im_best
        else:
        ...
        queue_valid_flag, center_point, non_zero_num, non_outlier_num = outlier_filter(pre_item.point_queue, min_valid_num=(5, 4))
